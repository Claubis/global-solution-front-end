...
const ModalDeletarConta: React.FC<ModalDeletarContaProps> = ({ isOpen, onClose }) => {
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="flex flex-col max-w-md gap-2 p-6 rounded-md shadow-md bg-white text-gray-800">
        <h2 className="flex items-center gap-2 text-xl font-montserrat font-semibold leading-tight tracking-wide">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" className="w-6 h-6 fill-current shrink-0 text-violet-600">
        ...
        <div className="flex flex-col justify-end gap-3 mt-6 sm:flex-row font-montserrat font-medium">
          ...
          <button className="px-6 py-2 rounded-sm shadow-sm bg-violet-600 text-gray-50">Deletar Conta</button>
        </div>
      </div>
    </div>
  );
